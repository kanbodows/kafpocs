#include "svodotchetpraktiki.h"
#include "ui_svodotchetpraktiki.h"
#include <QMessageBox>
#include <QList>
bool SvodOtchetPraktiki::isOpen = false;
SvodOtchetPraktiki::SvodOtchetPraktiki(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SvodOtchetPraktiki)
{
    ui->setupUi(this);
    if (!this->isOpen)
    {
...
SvodOtchetPraktiki::~SvodOtchetPraktiki()
{
    this->isOpen = false;
    delete ui;
}

void SvodOtchetPraktiki::printDocument(QPrinter *printer)
{
    body->print(printer);
}

void SvodOtchetPraktiki::on_pushButton_showReport_clicked()
{
    MyPrint myPrint;
    RowList list;
    list[3] = tr("Специальность");
    list[4] = tr("Тип практики");
    list[5] = tr("Группа");
    ...
    QString header = this->windowTitle() + "<br><br>" \
            "<H3>На период с " + ui->dateEdit_start->date().toString("dd-MM-yyyy") + " по " + ui->dateEdit_end->date().toString("dd-MM-yyyy") + "</H3>";
    body = myPrint.print(dal_studentsControl->getPractikiSvodReport(ui->comboBox_spec->model()->index(ui->comboBox_spec->currentIndex(),0).data().toInt(),
...
void SvodOtchetPraktiki::on_pushButton_graph_clicked()
{
    /*QLayoutItem* item;
